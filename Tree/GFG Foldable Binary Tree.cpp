...
bool issame(Node* l, Node* r){
    ...
    if(!l && !r) return true;
    // if both exist then check l->left and r->right, l->right an dr->left
    if(l && r)return issame(l->left, r->right) && issame(l->right, r->left);
    // otherwise false
    return false;
}
...
{
    // Your code goes here
    // id root will be null then true
    if(!root) return true;
    // otherwise check with is same function
